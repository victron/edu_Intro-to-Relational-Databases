...
def newMenu(restaurant_id):
    if request.method == 'POST':
        ...
        for restaurant in restaurants:
            if int(restaurant['id']) == restaurant_id:
                restaurant_name = restaurant['name']
                return render_template('newMenu.html', restaurant_name=restaurant_name, restaurant_id=restaurant_id)
    ...
    if request.method == 'POST':
        ...
        for item in items:
            if int(item['id']) == menu_id:
                ...
                items[id]['id'] = menu_id
                flash('menu %s changed' % item['name'])
                ...
                for item in items:
                    if int(item['id']) == menu_id:
                        return render_template('deleteMenu.html', restaurant_id=restaurant_id, menu_id=menu_id,
                                                restaurant_name=restaurant_name, item=item)
